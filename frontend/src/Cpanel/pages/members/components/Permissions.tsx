...
const ROLE_OPTIONS: Array<{ value: PermissionRole; label: string; note: string }> = [
  { value: 'business_owner', label: 'Workspace Owner', note: 'Full workspace access by default.' },
  { value: 'admin', label: 'Admin', note: 'Operational control across users, tasks, teams, and settings.' },
  { value: 'manager', label: 'Manager', note: 'Can manage teams and tasks, with limited workspace controls.' },
...
function buildDefaultPermissions(role: PermissionRole): PermissionMatrix {
  ...
  if (role === 'business_owner') {
    for (const group of ['user_management', 'task_management', 'team_management', 'settings'] as const) {
      for (const ability of Object.keys(empty[group])) {
        empty[group][ability as PermissionAbility] = true;
      }
    }
    empty.billing.view = false;
    empty.billing.manage = false;
    return {
      user_management: { view: true, create: true, edit: true, delete: true, invite: true, suspend: true },
      task_management: { view: true, create: true, edit: true, delete: true, manage: true },
      team_management: { view: true, create: true, edit: true, delete: true, manage: true },
...
function mapRole(memberRole: string | null | undefined): PermissionRole {
  if (memberRole === 'business_owner') return 'business_owner';
  if (memberRole === 'employee') return 'member';
  return 'member';
}

export default function MemberDetailsPermissionsTab() {
  const toast = useToast();
  const { member, refreshMemberDetails } = useMemberDetailsContext();

  const initialRole = useMemo(() => member?.permissions?.role ?? mapRole(member?.role), [member?.permissions?.role, member?.role]);
  const initialPermissions = useMemo(() => member?.permissions?.permissions ?? buildDefaultPermissions(initialRole), [initialRole, member?.permissions?.permissions]);
  ...
  const defaults = useMemo(() => buildDefaultPermissions(selectedRole), [selectedRole]);
  const roleMeta = ROLE_OPTIONS.find((item) => item.value === selectedRole);
  const isCustomOverride = JSON.stringify(permissions) !== JSON.stringify(defaults);
  ...
  async function handleSavePermissions() {
    ...
    try {
      setIsSaving(true);
      await saveMemberPermissions(member.id, {
        role: selectedRole,
        permissions,
        isCustomOverride,
      });
    ...
    <div className="rounded-xl border border-border bg-background p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Workspace access control for this member.</p>
        </div>
        <span
          className={`inline-flex rounded-full px-3 py-1.5 text-xs font-bold uppercase tracking-wide ${
            isCustomOverride ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
          }`}
        >
      ...
      <div className="mt-5 rounded-2xl border border-border bg-background p-4">
        ...
        <p className="mt-2 text-xs text-muted-foreground">{roleMeta?.note ?? 'This role grants a default permission set.'}</p>
      </div>
          ...
          <section key={group.id} className="rounded-2xl border border-border bg-background p-4">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {group.abilities.map((ability) => {
                ...
                return (
                  <label key={`${group.id}-${ability}`} className="flex cursor-pointer items-center justify-between rounded-xl border border-border px-3 py-2">
                    <span className="text-sm font-medium text-foreground">{humanizeAbility(ability)}</span>
                    <input
                      type="checkbox"
                      checked={checked}
