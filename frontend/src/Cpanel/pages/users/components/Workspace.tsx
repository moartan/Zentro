...
export default function UserDetailsWorkspaceTab() {
  ...
  const workspaceSlug =
    user?.role === "super_admin"
      ? "zentro"
      : primaryMembership?.businessSlug ?? "-";
  const workspaceOwner =
    user?.role === "super_admin"
      ? "Zentro Platform"
      : primaryMembership?.role === "business_owner"
        ? (user?.fullName ?? user?.email ?? "-")
        : "-";
  const joinedDate = formatDate(
    primaryMembership?.joinedAt ?? user?.createdAt ?? null,
  );
  const membershipRole = roleLabel(
    user?.role === "super_admin" ? "super_admin" : primaryMembership?.role,
  );
  const membershipStatus = memberStatusLabel(
    user?.role === "super_admin" ? "active" : primaryMembership?.status,
  );
  const plan =
    user?.role === "super_admin"
      ? "Enterprise"
  ...
  const subscriptionStatus =
    user?.role === "super_admin"
      ? "Active"
      : subscriptionStatusLabel(primaryMembership?.subscriptionStatus);
  ...
  async function handleChangeRole() {
    ...
    const nextRole =
      primaryMembership?.role === "employee" ? "business_owner" : "employee";
    const ok = window.confirm(`Change role to ${roleLabel(nextRole)}?`);
    if (!ok) return;

    try {
      setIsMutating(true);
      await adminChangeUserRole([user.id], nextRole);
  ...
  async function handleBlockToggle() {
    if (!user || user.role === "super_admin") return;
    const shouldBlock = membershipStatus !== "Blocked";
  ...
  return (
    <div className="rounded-xl border border-border bg-background p-6 shadow-sm">
      <div>
        <h2 className="text-xl font-semibold text-foreground">Business / Workspace</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Workspace-level details and controls for this user.
        </p>
      </div>
      ...
      <div className="mt-5 space-y-4">
        <Section title="Current Workspace">
          <div className="grid gap-3 lg:grid-cols-2">
            ...
            <Field label="Owner" value={workspaceOwner} />
        ...
        <Section title="Admin Actions">
          <div className="flex flex-wrap gap-2">
            <button
              ...
              disabled={isMutating || user?.role === "super_admin"}
            >
              {roleActionLabel}
            </button>
            <button
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Actions are disabled for platform super admin users.
          </p>
        </Section>
      </div>
    </div>
