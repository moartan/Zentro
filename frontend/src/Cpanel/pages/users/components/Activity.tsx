...
function entryTypePill(type: UserActivityEntry["type"]) {
...
export default function UserDetailsActivityTab() {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Sign-ins and audit trail for this user.
          </p>
        </div>
        <select
          value={filter}
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span
                    className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold uppercase tracking-wide ${entryTypePill(
                      entry.type,
                    )}`}
                  >
                    {entry.type}
                  ...
                  {entry.type === "login" && (
                    <span
                      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold uppercase tracking-wide ${loginStatusPill(
                        entry.success,
                      )}`}
                    >
